        ...
        return this.each(function ()
        {
            ...
            this.createHtml = function ()
            {
                ...
                if (para.dragDrop)
                {
                    ...
                    html += '				<div id="status_info" class="info">选中0张文件，共0B &nbsp;&nbsp;(最多上传' + maxfilenum + '张图片,格式'+imgsize+')</div>';
                    html += '				<div class="btns">';
                ...
                } else
                {
                    ...
                    html += '							<img class="upload_image" src="/components/zyupload/upload/add_img.png" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
                    html += '						</div>';
                    html += '					</a>';
                    html += '				</div>';
                    html += '			</div>';
            ...
            this.funSetStatusInfo = function (files)
            {
                ...
                // 设置内容
                $("#status_info").html("选中" + num + "张文件，共" + size + "&nbsp;&nbsp;(最多上传" + maxfilenum + "张图片,格式"+imgsize+")");
            };
            ...
            this.funFilterEligibleFile = function (files)
            {
                ...
                for (var i = 0, file; file = files[i]; i++)
                {
                    ...
                    {
                        layer.msg('图片"' + file.name + '"过大');
                    } else
                    {
                        // 在这里需要判断当前所有文件中
                        var fileExt = file.name.substr(file.name.lastIndexOf(".")+1).toLowerCase();//获得文件后缀名
                        var arr = new Array();
                        arr = filetype.split(',');
            ...
            this.funDisposePreviewHtml = function (file, e)
            {
                ...
                // 处理不同类型文件代表的图标
                var fileImgSrc = "/components/zyupload/fileType/";
                ...
                if (file.type.indexOf("image") == 0)
                {
                    ...
                    html += '		<div class="uploadImg" style="width:' + imgWidth + 'px;height:70px;overflow:hidden">';
                    html += '			<img id="uploadImage_' + file.index +
                        '" class="upload_image" src="' + e.target.result + '" style="height:70px;width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width);" />';
                    html += '		</div>';
                    html += '	</a>';
                ...
                } else
                {
                    ...
                    html += '			<img id="uploadImage_' + file.index + '" class="upload_image" src="' + fileImgSrc +
                        '" style="height:70px;width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width);" />';
                    html += '		</div>';
                    html += '	</a>';
                    html += '	<p id="uploadProgress_' + file.index + '" class="file_progress"></p>';
                    html += '	<p id="uploadFailure_' + file.index + '" class="file_failure">上传失败，请重试</p>';
...
{
    var num = 0;
    $.each($(".upload_append_list"), function (k, v)
    {
