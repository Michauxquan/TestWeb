...
namespace OWZX.Core
{
    ...
    public class CommonHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetFormateTime(object time)
        {
            string dt = DateTime.Parse(time.ToString()).ToString("yyyy-MM-dd HH:mm:ss");
            if (dt.Contains("1900"))
                return "";
            else
                return dt;
        ...
        /// 隐藏商品名称(返回截取的字符串，带...)
        /// </summary>
        public static string HideProductName(string proname,int length=-1)
        {
            if (proname != null && proname.Trim().Length > 0)
            {
                ...
                if (proname.Length > 30)
                {
                    if (length == -1)
                    {
                        return SubStrHelper.getString(proname, 30);
                    }
                    else
                        return SubStrHelper.getString(proname, length);
                }
                else
                {
                    if (length == -1)
        ...
        /// 隐藏商品名称(返回截取的字符串，不带...)
        /// </summary>
        public static string HideProductNameLimit(string proname, int length = -1)
        {
            if (proname != null && proname.Trim().Length > 0)
            {
                ...
                if (proname.Length > 30)
                {
                    if (length == -1)
                    {
                        return SubStrHelper.getLimitString(proname, 30);
                    }
                    else
                        return SubStrHelper.getLimitString(proname, length);
                }
                else
                {
                    if (length == -1)
        ...
        /// <returns></returns>
        public static Dictionary<string, string> ParmsToDic(string parms)
        {
            Dictionary<string, string> dic = new Dictionary<string, string>();
            foreach (string str in parms.Split('&'))
            { 
