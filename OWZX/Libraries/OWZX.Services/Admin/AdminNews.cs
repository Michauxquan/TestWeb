using System;
using System.Data;

using OWZX.Core;

namespace OWZX.Services
{
    /// <summary>
    /// 后台新闻操作管理类
    /// </summary>
    public partial class AdminNews : News
    {
        /// <summary>
        /// 创建新闻类型
        /// </summary>
        public static void CreateNewsType(NewsTypeInfo newsTypeInfo)
        {
            OWZX.Data.News.CreateNewsType(newsTypeInfo);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWSTYPE_LIST);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + "\\d+");
        }

        /// <summary>
        /// 删除新闻类型
        /// </summary>
        /// <param name="newsTypeId">新闻类型id</param>
        public static void DeleteNewsTypeById(int newsTypeId)
        {
            OWZX.Data.News.DeleteNewsTypeById(newsTypeId);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWSTYPE_LIST);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + "\\d+");
        }

        /// <summary>
        /// 更新新闻类型
        /// </summary>
        public static void UpdateNewsType(NewsTypeInfo newsTypeInfo)
        {
            OWZX.Data.News.UpdateNewsType(newsTypeInfo);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWSTYPE_LIST);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + "\\d+");
        }



        /// <summary>
        /// 创建新闻
        /// </summary>
        public static void CreateNews(NewsInfo newsInfo)
        {
            OWZX.Data.News.CreateNews(newsInfo);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + newsInfo.NewsTypeId);
            OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + "\\d+");
        }

        /// <summary>
        /// 删除新闻
        /// </summary>
        /// <param name="newsIdList">新闻id列表</param>
        public static void DeleteNewsById(int[] newsIdList)
        {
            if (newsIdList != null && newsIdList.Length > 0)
            {
                OWZX.Data.News.DeleteNewsById(CommonHelper.IntArrayToString(newsIdList));
                OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + "\\d+");
            }
        }

        /// <summary>
        /// 更新新闻
        /// </summary>
        public static bool UpdateNews(NewsInfo newsInfo)
        {
            bool result= OWZX.Data.News.UpdateNews(newsInfo);

            if (result)
            {
                OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + newsInfo.NewsTypeId);
                OWZX.Core.BSPCache.Remove(CacheKeys.SHOP_NEWS_HOMELIST + "\\d+");
                return true;
            }
            else
            {
                return false;
            }
        }

        /// <summary>
        /// 后台获得新闻
        /// </summary>
        /// <param name="newsId">新闻id</param>
        /// <returns></returns>
        public static NewsInfo AdminGetNewsById(int newsId)
        {
            if (newsId > 0)
                return OWZX.Data.News.AdminGetNewsById(newsId);
            return null;
        }

        /// <summary>
        /// 后台获得新闻列表
        /// </summary>
        /// <param name="pageSize">每页数</param>
        /// <param name="pageNumber">当前页数</param>
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public static DataTable AdminGetNewsList(int pageSize, int pageNumber, string condition)
        {
            return OWZX.Data.News.AdminGetNewsList(pageSize, pageNumber, condition);
        }

        /// <summary>
        /// 后台获得新闻列表搜索条件
        /// </summary>
        /// <param name="newsTypeId">新闻类型id</param>
        /// <param name="title">新闻标题</param>
        /// <returns></returns>
        public static string AdminGetNewsListCondition(int newsTypeId, string title)
        {
            return OWZX.Data.News.AdminGetNewsListCondition(newsTypeId, title);
        }

        /// <summary>
        /// 后台获得新闻数量
        /// </summary>
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public static int AdminGetNewsCount(string condition)
        {
            return OWZX.Data.News.AdminGetNewsCount(condition);
        }

        /// <summary>
        /// 后台根据新闻标题得到新闻id
        /// </summary>
        /// <param name="title">新闻标题</param>
        /// <returns></returns>
        public static int AdminGetNewsIdByTitle(string title)
        {
            if (string.IsNullOrWhiteSpace(title))
                return 0;
            return OWZX.Data.News.AdminGetNewsIdByTitle(title);
        }
    }
}
