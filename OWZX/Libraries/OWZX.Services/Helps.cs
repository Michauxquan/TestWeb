...
namespace OWZX.Services
{
    ...
    public partial class Helps
    {
        ...
        {
            List<HelpInfo> helpTree = OWZX.Core.BSPCache.Get(CacheKeys.SHOP_HELP_LIST) as List<HelpInfo>;

            if (helpTree == null)
            {
                helpTree = new List<HelpInfo>();
                List<HelpInfo> helpList = OWZX.Data.Helps.GetHelpList();

                CreateHelpTree(helpList, helpTree, 0);
                OWZX.Core.BSPCache.Insert(CacheKeys.SHOP_HELP_LIST, helpTree);
            }
            return helpTree;
        }
