...
namespace OWZX.Services
{
    ...
    public partial class Navs
    {
        ...
        {
            List<NavInfo> navTree = OWZX.Core.BSPCache.Get(CacheKeys.SHOP_NAV_LIST) as List<NavInfo>;

            if (navTree == null|| navTree.Count==0)
            {
                navTree = new List<NavInfo>();
                List<NavInfo> navList = OWZX.Data.Navs.GetNavList();

                CreateNavTree(navList, navTree, 0);
                OWZX.Core.BSPCache.Insert(CacheKeys.SHOP_NAV_LIST, navTree);
            }
            return navTree;
        }
        ...
        {
            List<NavInfo> navList = OWZX.Core.BSPCache.Get(CacheKeys.SHOP_NAV_MAINLIST) as List<NavInfo>;

            if (navList == null || navList.Count==0)
            {
                navList = new List<NavInfo>();
                foreach (NavInfo navInfo in OWZX.Data.Navs.GetNavList())
                {
                    if (navInfo.Pid == 0)
                        navList.Add(navInfo);
                }
                OWZX.Core.BSPCache.Insert(CacheKeys.SHOP_NAV_MAINLIST, navList);
            }
            return navList;
        }
