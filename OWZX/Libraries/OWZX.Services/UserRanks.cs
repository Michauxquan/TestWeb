...
namespace OWZX.Services
{
    ...
    public partial class UserRanks
    {
        ...
        {
            List<UserRankInfo> userRankList = OWZX.Core.BSPCache.Get(CacheKeys.SHOP_USERRANK_LIST) as List<UserRankInfo>;
            if (userRankList == null)
            {
                userRankList = OWZX.Data.UserRanks.GetUserRankList();
                OWZX.Core.BSPCache.Insert(CacheKeys.SHOP_USERRANK_LIST, userRankList);
            }
            return userRankList;
        }
