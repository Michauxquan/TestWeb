...
function init()
{
    ...
    $("#updiv").html("").zyUpload({
        ...
        url: "/admin/tool/upload?operation=uploadproductimage&cateid=" + $("#CateId").val(),  // 上传文件的路径
...
function buildEditor()
{
    ...
    var tableHtml = "";
    tableHtml += '<tr ><td width="84px" align="right">商品图片:</td><td></td><td></td></tr><tr>' +
                        '<!--上传商品图片-->'+
                        ...
                        '<td>'+
                            '<div id="updiv">'+
                            '</div>'+
                            '<input id="upimgurl" name="upimgurl" style="display:none;" value="" />'+
    ...
    '</script>'+
                        '</td>'+
                        '</tr> <tr ><td width="84px" align="right">商品详情:</td><td></td><td></td></tr><tr>' +
                    '<td width="84px" align="right"> ' +
                    '</td>'+
                    '<td>'+
                         '<script name="Description" id="bspEditor" type="text/plain"></script>'+
                         '<script type="text/javascript">'+
                          '       var cateid=$("#CateId").val();'+
                           '      window.UEDITOR_CONFIG.imageUrl = "" + "/admin/tool/upload?operation=uploadproducteditorimage&cateid="+cateid;'+
...
function selectSKUAttrValue() {
    ...
    for (var i = 1; i <= skuCount; i++) {
        ...
        for (var j = 0; j < productSKUItemList.length; j++) {
            ...
            skuListHtml += "<td width='88px'>" + productSKUItemList[j][index].attrValue + "<input type='hidden' name='SKUAttrIdList' value='" + productSKUItemList[j][index].attrId + "'/><input type='hidden' name='SKUAttrValueIdList' value='" + productSKUItemList[j][index].attrValueId + "'/><input type='hidden' name='SKUAttrInputValueList' value='" + productSKUItemList[j][index].inputValue + "'/></td>";
        }
    ...
    if (productPageType == 0 || productPageType == 3) {
        ...
        $.get("/admin/category/aandvjsonlist?cateId=" + selectedCateId + "&time=" + new Date(), function (data) {
            ...
            {   //添加商品页面时
                ...
                if (itcateid == -1)
                {
                    $("#prodetails").find("tr:not('.keepTr')").remove();
                    $("#prodetails").prepend(buildEditor());
                }
                itcateid = selectedCateId;
            }
...
$(function () {
    ...
    $(".submit").click(function ()
    {
        if ($("#CateId").val() == -1)
        {
            $.jBox.alert("请选择商品分类", "提示");
            return false
        }

        if ($("#BrandId").val() == -1)
        {
            $.jBox.alert("请选择商品品牌", "提示");
            return false
        }
        ...
        if (itcateid != -1)
        {
            if ($("#upimgurl").val() == "")
            {
                $.jBox.alert("请上传商品图片", "提示");
                return false
            }
            var text = UE.getEditor('bspEditor').getContent();
            if (text.length == 0)
            {
                $.jBox.alert("请输入商品详情", "提示");
                return false
            }
        }
        $("form:first").submit();
...
function buildimgs(imgs)
{
    ...
    {
        html += '<div style="display:inline-block;" id="proimgs_list' + i + '"><a href="javascript:delimg(\'' + ttimg[i].trim() + '\','+i+');" style="float:right;text-align:right;color:#0A6BA0">删除</a><br/><a style="padding:5px;" href="/upload/product/show/thumb350_350/' + cateid + '/' + ttimg[i] + '" target="_blank">' +
               '<img src="/upload/product/show/thumb100_100/' + cateid + '/' + ttimg[i].trim() + '" width="100px" height="100px">' +
               '</a></div>';
    }
...
{
   $.jBox.confirm("确定要删除该商品图片", "提示", function (v, h, f) {
        if (v == 'ok') {
