...
/*部门选择层开始*/
var openDepartSelectLayerBut = null;
var departSelectLayerHtml = "<div class='selectBoxProgressBar'><p><img src='/administration/images/progressbar.gif'/></p></div>";

function departTree(obj, layer)
{
    var state = $(obj).attr("class");
    if (state == "open")
    {
...
function setSelectedDepart(selectedDepId, selectedDepName)
{
    $(openDepartSelectLayerBut).parent().find(".DepId").val(selectedDepId);
    $(openDepartSelectLayerBut).val(selectedDepName).parent().find(".DepartName").val(selectedDepName);
    $.jBox.close('departSelectLayer');
}

function ajaxDepartSelectList()
{
    $.jBox.setContent(departSelectLayerHtml);
    $.get("/administration/menu/departlist.htm?t=" + new Date(), function (data)
    {
        $.jBox.setContent(data);
    })
}

function openDepartSelectLayer(openLayerBut)
{
    $.jBox('html:departSelectLayer', {
        id: 'departSelectLayer',
        width: 750,
        buttons: { '关闭': true },
        title: "选择部门"
    });
    openDepartSelectLayerBut = openLayerBut;
    ajaxDepartSelectList();
}
/*部门选择层结束*/
