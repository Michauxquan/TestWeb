...
function editUser() {
    ...
    var qqNum = userInfoForm.elements["qqNum"].value; 

    if (!verifyEditUser( nickName, realName)) {
        return;
    }

    $.post("/ucenter/edituser",
            { 'nickName': nickName, 'realName': realName, 'avatar': avatar, 'gender': gender,  'qqNum': qqNum },
            editUserResponse)
}

//验证编辑用户
function verifyEditUser( nickName, realName) {
